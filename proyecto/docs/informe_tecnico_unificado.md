...
## 2.3 Detalle del Taller 1 — Base de Conocimiento + Q&A clásico (Módulo 1)

**Objetivo del módulo**: construir el "cerebro" inicial del asistente extrayendo información pública de Sándwich Qbano, procesándola y exponiéndola como un sistema básico de preguntas y respuestas con prompt engineering.

**Lo que se construyó**:

- **Scraping HTML** de `sandwichqbano.com` (catálogo VTEX incluido) usando `BeautifulSoup` y `requests`. Cobertura: 25 páginas máximo configurables.
- **Parsing PDF** del Informe de Sostenibilidad 2023 con `pypdf`.
- **Consolidación de texto** en `data/processed/knowledge_base.txt` (~200 KB de texto limpio).
- **Chunking ad-hoc** (luego reemplazado en T2 por `RecursiveCharacterTextSplitter`).
- **Q&A con prompt engineering** en `src/chains.py` usando `ChatPromptTemplate` de LangChain.
- **UI Streamlit** con tres pestañas: Resumen, FAQ, Q&A clásico.
- **Prompts editables** desde la interfaz, guardados en `data/config/prompts_config.json`.
- **Validación** con 8 preguntas de prueba en `results/test_questions.csv`.

**Entregables del Taller 1**:

| Archivo | Para qué |
|---------|----------|
| `data/processed/knowledge_base.txt` | Conocimiento consolidado de la empresa |
| `data/processed/source_index.md` | Índice de fuentes usadas (sitio + PDF) |
| `pipeline_explicacion.md` | Documentación técnica del pipeline |
| `prompt_experimentacion.md` | Bitácora de iteración de prompts |
| `scripts/run_question_batch.py` | Validación batch del Q&A |

**Comentario del profesor que orientó el T2**: el approach de feature hashing custom utilizado en el T1 fue calificado como "el camino más difícil". Esa observación motivó la migración a componentes nativos de LangChain en el Taller 2.

## 2.4 Detalle del Taller 2 — Agente Conversacional con memoria y router (Módulo 2)

**Objetivo del módulo**: transformar el Q&A clásico del T1 en un agente conversacional con memoria, herramienta estructurada para datos puntuales, router que decide la fuente, y RAG vectorial real.

**Lo que se construyó encima del T1**:

- **Migración del vector store** de feature hashing a `langchain_chroma.Chroma` con `HuggingFaceEmbeddings` (`MiniLM-L12-v2`, 384 dim). Resuelve la deuda técnica heredada del T1.
- **Chunking profesional** con `RecursiveCharacterTextSplitter` (chunk_size=1000, overlap=150), reemplazando el chunking ad-hoc.
- **Tool estructurada** (`src/structured_tool.py`) que busca en `data/structured/company_structured_data.json` (43 registros canónicos con WhatsApp, redes sociales, PBX, sedes, etc).
- **Router determinístico** en `src/agent.py` con heurísticas regex que detectan tipo de pregunta antes de invocar al LLM.
- **Memoria conversacional** efímera en `st.session_state.agent_messages` para mantener contexto entre turnos.
- **Multi-LLM** configurable: Ollama (local), Google Gemini, OpenAI.
- **Pestaña "Agente conversacional"** en Streamlit con chat visible (`st.chat_message`).
- **Validación batch** con 33 casos en `results/agent_test_questions.csv`.

**Entregables del Taller 2**:

| Archivo | Para qué |
|---------|----------|
| `src/agent.py` | Router conversacional + selección de herramienta |
| `src/memory.py` | Manejo de memoria entre turnos |
| `src/structured_tool.py` | Tool para datos puntuales del JSON |
| `src/vector_store.py` | Capa ChromaDB + embeddings HuggingFace |
| `data/vector/chroma/chroma.sqlite3` | Vector store persistido (185 vectores) |
| `scripts/run_agent_batch.py` | Validación batch (33/33 OK) |
| `taller2/informe_taller2.md` | Informe de sustentación del T2 |

**Estado al cierre del T2**: 33/33 rutas correctas en el batch, ChromaDB con 185 vectores indexados, sistema funcional pero solo accesible vía Streamlit (sin canal externo, sin memoria persistente entre sesiones).

## 2.5 Deuda técnica resuelta entre Taller 2 y Taller 3
